...
def make_plot(ts,e,e_err,plot_title,filename,extrap=None):
    fig, ax = plt.subplots()
    ...
    if extrap is not None:
        ...
        ax.plot(line_ts, y, linestyle="--", label=r"$E=({}\pm{})\tau+({}\pm{})$".format(m,m_err,b,b_err),color='black')
...
for job in glob.glob("../data/*"):
    neutral_data = np.loadtxt("{}/neutral_energies.txt".format(job), skiprows=1, delimiter=",")
    anion_data = np.loadtxt("{}/anion_energies.txt".format(job), skiprows=1, delimiter=",")
    ...
    for i in [trialwfn, basis, separation]:
        i = i.replace("_", " ")
    ...
    neutral_title = "{} {} {} neutral".format(neutral_trialwfn, basis, separation, plot_title)
    anion_title = "{} {} {} anion".format(anion_trialwfn, basis, separation, plot_title)
    combined_title = "{} {} {} EBE".format(trialwfn, basis, separation, plot_title)
    ...
    make_plot(ts, neutral_energies, neutral_err, neutral_title, "{}_01_neutral.png".format(filename), neutral_extrap_data)
    make_plot(ts, anion_energies, anion_err, anion_title, "{}_02_anion.png".format(filename),anion_extrap_data)
    make_plot(ts, ebe, ebe_err, combined_title, "{}_03_ebe.png".format(filename))
